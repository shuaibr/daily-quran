...
def _test_pause_unpause_button(job: Job, button_label: str) -> None:
    button2: test.Button | None = _get_pause_or_unpause_button(job)
    assert button2
    assert button2.label == button_label
    assert button2.style == test.ButtonStyle.PRIMARY
    assert button2.type == test.ComponentType.BUTTON
...
class TestCountdown:
    ...
    parsed_date: Union[datetime, None] = dateparser.parse(
        "18 January 2040",
        settings={
    ...
    def test_make_button(self) -> None:  # noqa: ANN101
        ...
        button: test.Button = _make_button(label=button_name, style=test.ButtonStyle.PRIMARY)
        assert button.label == button_name
        assert button.style == test.ButtonStyle.PRIMARY
    ...
    def test_get_pause_or_unpause_button(self) -> None:  # noqa: ANN101
        button: test.Button | None = _get_pause_or_unpause_button(self.normal_job)
    ...
    def test_get_row_of_buttons(self) -> None:  # noqa: ANN101
        row: test.ActionRow = _get_row_of_buttons(self.normal_job)
        assert row
        ...
        row2: test.ActionRow = _get_row_of_buttons(self.cron_job)
        assert row2
    ...
    def test_get_pages(self) -> None:  # noqa: ANN101
        ...
        channel: test.Channel = test.Channel(id=test.Snowflake(865712621109772329))
