...
def _get_trigger_text(job: Job) -> str:
    ...
    trigger_time: Union[datetime, None] = job.trigger.run_date if isinstance(job.trigger, DateTrigger) else job.next_run_time
...
def _get_pause_or_unpause_button(job: Job) -> Button | None:
    ...
    if type(job.trigger) is not DateTrigger:
        pause_button: Button = _make_button("Pause", test.ButtonStyle.PRIMARY)
        unpause_button: Button = _make_button("Unpause", test.ButtonStyle.PRIMARY)
...
def _get_row_of_buttons(job: Job) -> ActionRow:
    ...
    components: list[Button] = [
        _make_button("Edit", test.ButtonStyle.PRIMARY),
        _make_button("Remove", test.ButtonStyle.DANGER),
    ]
...
def _remove_job(job: Job) -> str:
    ...
    try:
        ...
        trigger_time: Union[datetime, str] = job.trigger.run_date
...
async def _callback(self: Paginator, ctx: ComponentContext) -> Message | None:
    ...
    job_id: Union[str, None] = self.component_ctx.message.embeds[0].title
    ...
    components: list[TextInput] = [
        test.TextInput(
            style=test.TextStyleType.PARAGRAPH,
            label="New message",
    ...
    if type(job.trigger) is DateTrigger:
        ...
        trigger_time: Union[datetime, None] = job.trigger.run_date
        job_type: str = "normal"
        components.append(
            test.TextInput(
                style=test.TextStyleType.SHORT,
