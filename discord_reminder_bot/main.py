...
bot: Client = test.Client(token=DISCORD_TOKEN)
...
async def base_command(ctx: test.CommandContext) -> None:  # noqa: ARG001
...
async def modal_response_edit(ctx: CommandContext, *response: str) -> Message:  # noqa: C901, PLR0912, PLR0911
    ...
    job_id: Union[str, None] = ctx.message.embeds[0].title
    old_date: Union[str, None] = None
    old_message: Union[str, None] = None
    ...
    if type(job.trigger) is DateTrigger:
        new_message: Union[str, None] = response[0]
        new_date: Union[str, None] = response[1]
    else:
    ...
    if old_date is not None and new_date:
        ...
        parsed_date: Union[datetime, None] = parsed.parsed_time
...
async def parse_command(
    ctx: test.CommandContext,
    time_to_parse: str,
    optional_timezone: Union[str, None] = None,
) -> Message:
...
async def list_command(ctx: test.CommandContext) -> Message | None:
...
async def command_add(  # noqa: PLR0913
    ...
    message_reason: str,
    message_date: str,
    different_channel: test.Channel | None = None,
    send_dm_to_user: test.User | None = None,
    both_dm_and_channel: Union[bool, None] = None,
) -> Message | None:
...
async def remind_cron(  # noqa: PLR0913
    ...
    different_channel: Union[test.Channel, None] = None,
    send_dm_to_user: Union[test.User, None] = None,
    both_dm_and_channel: Union[bool, None] = None,
) -> None:
...
async def send_to_discord(channel_id: int, message: str, author_id: int) -> None:
    ...
    """
    channel: Channel = await test.get(
        bot,
        test.Channel,
        object_id=channel_id,
        force=test.Force.HTTP,
    )
